...
class NonStationaryMabEnvironment:
    ...
    def computeAbruptChanges(self):
        for i in range(1, self.bids.shape[0]):
            if np.random.random() < self.changeProbability:
                self.bids[i] = np.random.randint(0, auctionHouse.MAX_BID + 1, auctionHouse.NB_CATEGORIES)
    ...
    def round(self, pulled_arm, nbAdvertiser=0):
        ...
        # now that time passed, the behavior of advertisers could change
        self.computeAbruptChanges()
